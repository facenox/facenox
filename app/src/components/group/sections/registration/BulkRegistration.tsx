    ...
    <div className="relative flex h-full flex-col overflow-hidden">
      {pendingDuplicates && (
        <>
          ...
          <div className="intro-y absolute top-1/2 left-1/2 z-50 max-w-[95%] min-w-[360px] -translate-x-1/2 -translate-y-1/2 rounded-3xl border border-amber-500/30 bg-[#18181b] p-6">
            ...
            <div className="flex gap-2">
              ...
              {pendingDuplicates.newFiles.length > 0 && (
                <button
                  ...
                  className="flex-1 rounded-lg border border-white/5 bg-white/5 px-4 py-2 text-[11px] font-medium text-white transition-all hover:bg-white/10">
                  Skip
                </button>
              )}
              <button
                onClick={() => void handleConfirmDuplicates()}
                className="flex-1 rounded-lg border border-amber-500/20 bg-amber-500/10 px-4 py-2 text-[11px] font-bold tracking-wider text-amber-500 transition-all hover:bg-amber-500/20">
                Add anyway
              </button>
            </div>
          </div>
